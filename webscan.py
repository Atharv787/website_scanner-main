...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/WebScan_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/WebScan_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/WebScan_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/WebScan_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/WebScan_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o WebScan_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    os.system('wget -N https://raw.githubusercontent.com/skavngr/WebScan/master/WebScan.py -O WebScan.py > /dev/null 2>&1')
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/WebScan_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory WebScan resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/WebScan_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
